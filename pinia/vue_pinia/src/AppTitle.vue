...
<template>
  ...
  <header class="header">
    <a class="logo" href="https://vuejs.org/" target="_blank">
      <span class="img-inline"></span>
    </a>
    <h1>
      Vue
      <span style="color: #cbd71b"> Pinia </span>
      - <span id="exerciseNumber"> (Student Template)</span>
    </h1>
    <svg
      @click="startBlinking"
...
let blinking = ref("open"); // open, closed
const blinkTimer = 100;
const talking = ref("closed"); // open, closed
...
const startBlinking = () => {
  ...
  function blinkHandler() {
    ...
    if (blinkState % 2) {
      ...
      talking.value = "open";
      timerId = setTimeout(blinkHandler, blinkTimer * 1.7);
    } else if (blinkState < 4) {
      talking.value = "closed";
      blinking.value = "open";
      timerId = setTimeout(blinkHandler, blinkTimer);
    } else {
      blinking.value = "open";
